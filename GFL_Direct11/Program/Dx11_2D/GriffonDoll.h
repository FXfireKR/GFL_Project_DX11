...
{
private:
	map<UINT, IOPtaticDoll*> mAlltacDoll;		//모든 전술인형 [획득순서로 분류됨]
...
public:
	...
	//Battle TaticDoll
	HRESULT Create_IOPtacDoll(IOPDOLL_NAME name);				//전술인형을 생산한다

	HRESULT InsertSquad_IOPtacDoll(UINT squadId, UINT allid);	//전술인형을 분대에 넣는다.
	HRESULT ExitSquad_IOPtacDoll(UINT squadId, UINT memID);		//전술인형을 분대에서 퇴출한다
	HRESULT AppointLeader_IOPsquad(UINT sid, UINT mid);			//분대내의 전술인형을 분대장으로 임명한다.
