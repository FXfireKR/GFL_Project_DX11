...
void SoundManager::InsertSoundBianry(IN string key, IN string _path)
{
	if (SoundEngine)
	{
		if (!mSoundRes.count(key))
...
void SoundManager::Play_Sound(SOUND_CHANNEL ch, string key, float volume)
{
	...
	{
		miSoundRes->second->resource->setDefaultVolume(volume);
		//mChannel[ch]->curInfo = mChannel[ch]->engine->play2D(miSoundRes->second->resource, false, true, true);
	}
}