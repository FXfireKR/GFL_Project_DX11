#ifndef DXCOMPONENT_H
#define DXCOMPONENT_H
...
constexpr int PCElementCount = 2;
...
constexpr int PTElementCount = 2;
...
constexpr int PNTElementCount = 3;
...
//	컴퓨트 쉐이더 버퍼 - CPU Read 생성
inline void CreateComputeBuffer_Read(ID3D11Buffer** buffer, UINT byteWidth, UINT byteStride)
{
	...
	assert(SUCCEEDED(hr));
#else
	//Device->CreateBuffer(&desc, NULL, ppBuffer);
#endif
	
}

//	컴퓨트 쉐이더 버퍼 생성
inline void CreateComputeBuffer(ID3D11Buffer** buffer, UINT byteWidth, UINT byteStride, const void* pSystem)
{
	...
	if (pSystem == NULL || pSystem == nullptr) {
		HRESULT hr = Device->CreateBuffer(&desc, NULL, buffer);
		assert(SUCCEEDED(hr));
	}
	...
	//	Device->CreateBuffer(&desc, NULL, ppBuffer);
	//else
	//	Device->CreateBuffer(&desc, &data, ppBuffer);
#endif
}
