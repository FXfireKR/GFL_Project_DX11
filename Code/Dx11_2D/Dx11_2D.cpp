...
WCHAR		szTitle[MAX_LOADSTRING] = L"小女戰線";		// 제목 표시줄 텍스트입니다.
POINT		ptAdjWinSize;
	...
	while (msg.message != WM_QUIT)
	{
		...
		else
		{
			...
			if (!appPaused)
			{
				...
				ImGui::Update();
				ImGui::Text("FPS : %.2f", TIMER->FPS());
				ImGui::Text("mspl : %.2f", TIMER->ElaspedTime());
				ImGui::Text("worldTime : %.2f", TIMER->GameTime());
				ImGui::Text("Delta : %.2f", TIMER->DeltaTime());
...
#ifdef _DEBUG
	static int a = _CrtDumpMemoryLeaks();
#endif
...
void InitInstance(HINSTANCE hInstance, int nCmdShow)
{
   ...
   // 화면 해상도 얻기
	...
	RECT rt = { nWinPosX, nWinPosY, static_cast<LONG>(nWinPosX + WINSIZEX), static_cast<LONG>(nWinPosY + WINSIZEY) };
...
void InitDirectX(HINSTANCE hInstance)
{
	...
	//Create g_pDevice and g_pDeviceContext, g_pSwapChain
	{
		...
		hr = pOutput->GetDisplayModeList(format, DXGI_ENUM_MODES_INTERLACED, &numModes, NULL);
		...
		swapDesc.BufferDesc.RefreshRate.Numerator = displayModes->RefreshRate.Numerator;
		swapDesc.BufferDesc.RefreshRate.Denominator = displayModes->RefreshRate.Denominator;
