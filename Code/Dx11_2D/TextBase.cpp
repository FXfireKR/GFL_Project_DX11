...
HRESULT TextBase::Init(LPCWSTR fontname, float size, IDWriteFactory ** factory, IDWriteTextFormat ** format, DWRITE_FONT_WEIGHT height, 
	DWRITE_FONT_STYLE fontStyle, DWRITE_FONT_STRETCH strech)
{
	return (*factory)->CreateTextFormat(fontname, 0, height, fontStyle, strech, size, L"ko", format);
}

HRESULT TextBase::CreateTextBase(LPCWSTR fntName, float size, 
	DWRITE_FONT_WEIGHT height, DWRITE_FONT_STYLE fontStyle, DWRITE_FONT_STRETCH strech)
{
	this->Init(fntName, size, &dwFactory, &pTextFormat, height, fontStyle, strech);
...
HRESULT TextBase::DelayText(UINT cnt)
{
	...
	count = 0;
	maxCount = static_cast<float>(cnt);
