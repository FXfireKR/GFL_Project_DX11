...
void TutorialScene::init()
{
	if (!Initialize)
	{
		...
		/*
			@	기존에 존재하던 1분대를 임시 세이브
		*/

		vIDSave.clear();
		vIDSave.reserve(PLAYER->getPlayerSquad(PLAYER_TARGET_SQUAD)->squadMember.size());

		for (auto& iter : PLAYER->getPlayerSquad(PLAYER_TARGET_SQUAD)->squadMember) {
			BaseTaticDoll* itTacDoll = iter.second;
			vIDSave.push_back(itTacDoll->getID()->All_ID);	
		}
		for (size_t i = 0; i < vIDSave.size(); ++i)
			PLAYER->deleteDollToSquad(PLAYER_TARGET_SQUAD, 0);
		...
		PLAYER->insertDollToSquad(curTacDoll->getID()->All_ID, 1);
		...
		LOAD->setNextScene("TUTORIAL");

		LOAD->mallocThread();
		SCENE->changeScene("LOAD");

		PLAYER->setTechKeyLock(true);
...
void TutorialScene::unLoad()
{
	...
	//	delete Squad Member
	PLAYER->deleteDollToSquad(curTacDoll->getID()->Squad_ID, curTacDoll->getID()->SquadMem_ID);
	PLAYER->setTechKeyLock(false);

	for (size_t i = 0; i < vIDSave.size(); ++i)
		PLAYER->insertDollToSquad(vIDSave[i], PLAYER_TARGET_SQUAD);	
	...
	if (!lockConv)
	{
		if (textCount < vCon.size() - 1)
		{
			if (DWRITE->getProperty("CONV")->getdelayRendOver())
			{
				if (KEYMANAGER->isKeyUp(VK_SPACE))
				{
					...
					DWRITE->Create_TextField("NAME", TUTORIAL_TEXT_FONT, vCon.at(textCount).SpeakName, 30.0f);
					DWRITE->Create_TextField("CONV", TUTORIAL_TEXT_FONT, vCon.at(textCount).text, 20.0f);
					DWRITE->Create_TextField("CONV_fix", TUTORIAL_TEXT_FONT, vCon.at(textCount).text, 20.0f);
		...
		else
		{
			if (KEYMANAGER->isKeyUp(VK_SPACE))
			{
				...
				LOAD->setStoryScriptPath("ChapterScript_0_1");
...
void TutorialScene::scriptRender()
{
	...
	{
		if (bkKey.size() > 0)
			DRAW->render(bkKey, VEC2(WINSIZEX*0.5f, WINSIZEY*0.5f), D3DXVECTOR2(WINSIZEX*0.5F, WINSIZEY*0.5F));
		...
		if (textCount != 0)
		{
			if (vCon.at(textCount - 1).vImageKey.size() > 0)
			{
				...
				DRAW->render(speaker, speaker_alpha, VEC2(1024 * 0.7f, 1024 * 0.7f),
		...
		DRAW->render(atlas->textureKey, atlas->alphaTexKey, VEC2(375, 100), VEC2(TUTORIAL_BOX_POS_X + 375, TUTORIAL_BOX_POS_Y + 80),
			atlas->mixTexCoord, atlas->maxTexCoord, D3DXCOLOR(1, 1, 1, 0.8f));

		DWRITE->TextRender("NAME", TUTORIAL_BOX_POS_X + 20 + 10, TUTORIAL_BOX_POS_Y - 15 + 10, { 255.0F, 255.0F, 0.0F });
		...
		if (!DWRITE->getProperty("CONV")->getdelayRendOver())
		{
			DWRITE->TextRender("CONV", TUTORIAL_BOX_POS_X + 20, TUTORIAL_BOX_POS_Y + 50,
		...
		{
			DWRITE->TextRender("CONV_fix", TUTORIAL_BOX_POS_X + 20, TUTORIAL_BOX_POS_Y + 50,
				TUTORIAL_TEXT_WIDTH - 40, TUTORIAL_TEXT_HEIGHT, { 255.0F, 255.0F, 255.0F });
...
void TutorialScene::readScript()
{
	...
	while (!file.eof())
	{
		...
		USES_CONVERSION;
		wstring wBuffer = A2W(buffer.c_str());
		...
		//	Story모드 종료시 행동 요령
		if (wBuffer.find(L"《") != wstring::npos)
		{
			//	1. 다음 Scene 배정
		...
		else if (wBuffer.find(L"》") != wstring::npos)
		{
			string value;
