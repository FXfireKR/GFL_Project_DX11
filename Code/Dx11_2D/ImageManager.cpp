...
ImageManager::ImageManager()
	: mUiAtlas(nullptr)
{
	mUiAtlas = new unordered_map<string, uiAtlas*>;
}
...
void ImageManager::InsertImageBinary(ThreadPool* _trdPol, string key, string _path)
{
	if (_trdPol != nullptr)
	{
		if (!mResourece.count(key))
		{
			...
			int onceSize = static_cast<int>(static_cast<float>(_trdPol->CipByte.size()) * 0.03125f);
...
void ImageManager::ReleaseMemory(string key)
{
	if ((miResourece = mResourece.find(key)) != mResourece.end()) {
		...
		if (miResourece->second != nullptr) {

			SAFE_RELEASE(miResourece->second->texture);
			SAFE_DELETE(miResourece->second);
		}
		mResourece.erase(miResourece);
	}
}