...
{
	DRAW->render(bkKey, DV2(WINSIZEX, WINSIZEY), D3DXVECTOR2(WINSIZEX*0.5F, WINSIZEY*0.5F));
	...
	{ 
		if (vCon.at(textCount - 1).vImageKey.size() > 0)
		{
			...
			DRAW->render(speaker, speaker_alpha, DV2(1024 * TEXT_SPEAKER_ACL, 1024 * TEXT_SPEAKER_ACL),
				DV2(WINSIZEX * 0.5F, WINSIZEY * 0.7F), D3DXCOLOR(1, 1, 1, opc));

			//if (IMAGEMAP->getTexture(vCon.at(textCount - 1).vImageKey.at(0)) != nullptr)
