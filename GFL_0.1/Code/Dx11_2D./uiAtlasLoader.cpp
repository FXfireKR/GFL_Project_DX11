...
string uiAtlasLoader::readString(const FILE * file)
{
	return Tokenize(file).second;
}
...
int uiAtlasLoader::readInteger(const FILE * file)
{
	return atoi(Tokenize(file).second.c_str());
}

bool uiAtlasLoader::readBoolean(const FILE * file)
{
	return isFind(Tokenize(file).second, "true");
}

bool uiAtlasLoader::isCompare(const string str, const char * comp)
{
	return (str.compare(comp)) ? false : true;
}

bool uiAtlasLoader::isFind(const string str, const char * comp)
{
	return (str.find(comp) != string::npos) ? true : false;
}

const map<string, uiAtlas>& uiAtlasLoader::LoaduiAtlasData(const char * _path, map<string, uiAtlas>* _dataBase)
{
	if (_dataBase == nullptr)
		_dataBase = new map<string, uiAtlas>;
	...
	if (file != NULL)
	{
		...
		path.append(".ab");
		LOADMANAGER->Add_LoadTray(normalKey, path.c_str(), LOADRESOURCE_TYPE::RESOURCE_IMAGE);
		path.clear();
...
{
	map<string, uiAtlas>* _newDataBase = IMAGEMAP->getUiAtlas();

	if (_newDataBase == nullptr)
		_newDataBase = new map<string, uiAtlas>;
	...
	if (file != NULL)
	{
		...
		path.append(AlphaKey);
		path.append(".ab");
		LOADMANAGER->Add_LoadTray(AlphaKey, path.c_str(), LOADRESOURCE_TYPE::RESOURCE_IMAGE);
