...
void PlayerData::insertTacDolToSquad(SINT _allId, SINT _squadId)
{
	//	전체 멤버가 있다면
	if (tacDoll->getAllTacDoll().count(_allId))
		tacDoll->InsertSquad_IOPtacDoll(_squadId, _allId);
}

void PlayerData::deleteTacDolToSquad(SINT _squadMemId, SINT _squadId)
{
	//	해당 스쿼드에 멤버가 존재한다면
	if (tacDoll->getSquad(_squadId)->mSquad.count(_squadMemId))
		tacDoll->ExitSquad_IOPtacDoll(_squadId, _squadMemId);
}